"""
Model Architecture Ablation Studies for KGScout.

Runs 4 ablation experiments removing one architectural component at a time:
1. no-ppr:  Model without PPR (Personalized PageRank) scores
2. no-rt:   Model without Relation Tower
3. no-tt:   Model without Triplet Tower
4. no-gate: Model without Gating mechanism

Each experiment: pretrain (n=500, 5 epochs) -> train (k=1000, n=100, 30 epochs) -> inference (k=1000, top_k=100)
Results saved to ./results/model-ablation/<variant>/
"""
...
logger = logging.getLogger("ablation.model")
...
class PathRankingModelNoPPR(nn.Module):
    """Model WITHOUT PPR scores. MLPs take hidden_size*3 (no +2 for PPR features)."""
    def __init__(self, hidden_size=384, device="cuda"):
        super().__init__()
        self.device = device
        self.hidden_size = hidden_size
...
class PathRankingModelNoRT(nn.Module):
    """Model WITHOUT Relation Tower. Only triplet tower + gate in combiner."""
    def __init__(self, hidden_size=384, device="cuda"):
        super().__init__()
        self.device = device
        self.hidden_size = hidden_size
        ...
        self.triplet_mlp = nn.Sequential(
            ...
            nn.Linear(hidden_size // 2, 1))
        # No relation_mlp
        # Combiner takes 2 inputs: tower_A + gate
        self.combiner_mlp = nn.Sequential(
            nn.Linear(2, hidden_size // 2), nn.ReLU(), nn.Dropout(0.1),
            nn.Linear(hidden_size // 2, 1))
        self.temperature = nn.Parameter(torch.ones(1) * 1.0)
        self.baseline = nn.Parameter(torch.zeros(1))
    ...
    def forward(self, question_embed, triplet_embeds, relation_embeds, graph_scores):
        ...
        question_embed = question_embed.unsqueeze(0) if question_embed.dim() == 1 else question_embed
        triplet_attended, _ = self.question_triplet_attention(triplet_embeds, question_embed, question_embed)
        # Still compute relation attention for gate input
        relation_attended, _ = self.question_relation_attention(relation_embeds, question_embed, question_embed)
        question_expanded = question_embed.expand(num_triplets, -1)
        gate_input = torch.cat([question_expanded, triplet_embeds, relation_embeds], dim=-1)
        path_gates = self.gate_network(gate_input).squeeze(-1)
...
class PathRankingModelNoTT(nn.Module):
    """Model WITHOUT Triplet Tower. Only relation tower + gate in combiner."""
    def __init__(self, hidden_size=384, device="cuda"):
        super().__init__()
        self.device = device
        self.hidden_size = hidden_size
        ...
        self.relation_mlp = nn.Sequential(
            ...
            nn.Linear(hidden_size, hidden_size // 2), nn.ReLU(),
            nn.Linear(hidden_size // 2, 1))
        # Combiner takes 2 inputs: tower_B + gate
        self.combiner_mlp = nn.Sequential(
            nn.Linear(2, hidden_size // 2), nn.ReLU(), nn.Dropout(0.1),
            nn.Linear(hidden_size // 2, 1))
        self.temperature = nn.Parameter(torch.ones(1) * 1.0)
...
class PathRankingModelNoGate(nn.Module):
    """Model WITHOUT Gate network. Combiner takes only tower_A + tower_B."""
    def __init__(self, hidden_size=384, device="cuda"):
        super().__init__()
        self.device = device
        self.hidden_size = hidden_size
        ...
        self.relation_mlp = nn.Sequential(
            ...
            nn.Linear(hidden_size, hidden_size // 2), nn.ReLU(),
            nn.Linear(hidden_size // 2, 1))
        # Combiner takes 2 inputs: tower_A + tower_B (no gate)
        self.combiner_mlp = nn.Sequential(
            nn.Linear(2, hidden_size // 2), nn.ReLU(), nn.Dropout(0.1),
            nn.Linear(hidden_size // 2, 1))
        self.temperature = nn.Parameter(torch.ones(1) * 1.0)
...
class PathRankingModelNoRA(nn.Module):
    """
    Model WITHOUT Relation Attention, Gate, Relation Tower, and Combiner.
    Only triplet attention + triplet tower remain. Output is directly tower_A scores.
    """
    def __init__(self, hidden_size=384, device="cuda"):
        super().__init__()
        self.device = device
        self.hidden_size = hidden_size
        # Only triplet attention remains
        self.question_triplet_attention = nn.MultiheadAttention(
            embed_dim=self.hidden_size, num_heads=8, batch_first=True, dropout=0.1)
        # No relation attention, no gate, no relation tower, no combiner
        # Only triplet tower
        self.triplet_mlp = nn.Sequential(
            nn.Linear(hidden_size * 3 + 2, hidden_size),
            nn.LayerNorm(hidden_size), nn.ReLU(), nn.Dropout(0.1),
...
class PathRankingModelNoTA(nn.Module):
    """
    Model WITHOUT Triplet Attention, Gate, Triplet Tower, and Combiner.
    Only relation attention + relation tower remain. Output is directly tower_B scores.
    """
    def __init__(self, hidden_size=384, device="cuda"):
        super().__init__()
        self.device = device
        self.hidden_size = hidden_size
        # Only relation attention remains
        self.question_relation_attention = nn.MultiheadAttention(
            embed_dim=self.hidden_size, num_heads=8, batch_first=True, dropout=0.1)
        # No triplet attention, no gate, no triplet tower, no combiner
        # Only relation tower
        self.relation_mlp = nn.Sequential(
            nn.Linear(hidden_size * 3 + 2, hidden_size),
            nn.LayerNorm(hidden_size), nn.ReLU(), nn.Dropout(0.1),
...
def _sample_paths_impl(probabilities, paths, k, ranking_scores):
    """Shared sample_paths implementation for all model variants."""
    if len(paths) <= k:
...
# ============================================================================
# REWARD FUNCTION (original compute_reward_v8)
# ============================================================================

def compute_reward_v8(triplets, q_entities, a_entities, lambda_lin=0.2):
    """Original reward function with all 3 components."""
    if not triplets:
        return 0.0
    G = nx.DiGraph()
    ...
    total = w_pres * frac_presence + w_conn * conn_score + w_cov * path_cov
...
class JointTrainer:
    """Trains the path ranker using REINFORCE with compute_reward_v8."""
...
# ============================================================================
# MAIN: RUN MODEL ABLATION EXPERIMENTS
# ============================================================================

MODEL_ABLATION_CONFIGS = {
    "no-ppr": {
        "model_class": PathRankingModelNoPPR,
        "description": "Model without PPR scores"
    },
    "no-rt": {
        "model_class": PathRankingModelNoRT,
        "description": "Model without Relation Tower"
    },
    "no-tt": {
        "model_class": PathRankingModelNoTT,
        "description": "Model without Triplet Tower"
    },
    "no-gate": {
        "model_class": PathRankingModelNoGate,
        "description": "Model without Gating mechanism"
    },
    "no-ra": {
        "model_class": PathRankingModelNoRA,
        "description": "Model without Relation Attention (removes relation attention, gate, relation tower, combiner)"
    },
    "no-ta": {
        "model_class": PathRankingModelNoTA,
        "description": "Model without Triplet Attention (removes triplet attention, gate, triplet tower, combiner)"
    },
}
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="Run model architecture ablation studies")
    ...
    parser.add_argument("--output_dir", type=str, default="./results/model-ablation", help="Output directory")
