...
Tests swapping Query/Key-Value in the cross-attention modules:
  Original: Query=Triplets(N,d), Key=Value=Question(1,d) → output identical across rows
  Reversed: Query=Question(1,d), Key=Value=Triplets(N,d) → question attends over triplets

Pipeline:
  1. Load CWQ/WebQSP preprocessed data
  2. Pretrain (n=500, 5 epochs)
  3. Train with REINFORCE (n=1000, k=100, 30 epochs)
  4. Run coverage analysis (answer_coverage, path_coverage)
  5. Run LLM inference (Llama-3.1-8B) and report Hit, Hit@1, F1, Precision, Recall

Usage:
    python ablation-2/run_reversed_attention.py \
        --dataset cwq \
        --train-data /path/to/train.pt \
        --val-data /path/to/val.pt \
        --test-data /path/to/test.pt \
        --output-dir results/ablation-2/cwq
"""
...
def main():
    ...
    parser.add_argument("--output-dir", type=str, default="results/ablation-2/")
